...
def waveBody(obj, ):
    ...
    for node in obj.nodes:
        obj.nodes[node][0] += -20. * np.sin(obj.nodes[node][1] / 100. * 2 * np.pi)
...
if __name__ == "__main__":
    ...
    deformMethod = {"1": "shear", "2": "wave"}[
        input("\033[35;1m which deformed method do you want? 1: shear, 2: wave\n insert 1 or 2: \033[0m")
    ]
    if deformMethod == "shear":
        shearBody(obj)
    elif deformMethod == "wave":
        waveBody(obj)
    ...
    if writeInp == 'y':
        newFileName = path + "{}_{}.inp".format(job, deformMethod) 
